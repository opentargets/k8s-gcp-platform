locals {
  ...
  ppp_images_idx_start_ch = len(local.ppp_images_raw_ch) <= 3 ? 0 : len(local.ppp_images_raw_ch) - 3
  // This is the effective listing of PPP Clickhouse images to deploy
  ppp_images_list_ch = slice(local.ppp_images_raw_ch, local.ppp_images_idx_start_ch, len(local.ppp_images_raw_ch))
  ...
  ppp_images_idx_start_os = len(local.ppp_images_raw_os) <= 3 ? 0 : len(local.ppp_images_raw_os) - 3
  // This is the effective listing of PPP OpenSearch images to deploy
  ppp_images_list_os = slice(local.ppp_images_raw_os, local.ppp_images_idx_start_os, len(local.ppp_images_raw_os))
}